...
class Order_model extends CI_Model {
     ...
     * @param number $offset
     */
    public function getOrder($select='*',$where=array(),$order = array(), $limit = 10, $offset = 0){
    	$this->db->select($select);
    	$this->db->where($where);
    	if (!empty($order)) {
    		$this->db->order_by($order);
    	...
    	//$this->db->from($this->_table);
    	$this->db->join('tyo_product', 'tyo_product.id = tyo_order.product_id', 'left');
    	return $this->db->get($this->_table, $limit, $offset);
